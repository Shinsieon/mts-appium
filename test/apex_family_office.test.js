...
describe("APEX Family Office Test", function() {
    ...
    it('아들(김신영)에게 결혼 기념<임신축하금>으로 100만원을 2024년 03월 15일에 증여합니다. ', async function(){
        ...
        await loginPage.clickElement(await loginPage.findElementWithRetry(new Selector('Button', 'text', '전체메뉴').getSelector()))
        ...
        await apexPage.clickElement(await apexPage.findElementWithRetry(new Selector(comp.button, attribute.text, "확인").getSelector()));
        ...
        expect(await btnReq.getText()).to.equal('완료');
